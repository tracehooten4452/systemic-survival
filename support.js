...
(() => {
  ...
  function loadReactUmd() {
    ...
    const loadLocal = () => Promise.all([
      loadScript(REACT_LOCAL_URL, REACT_SRI),
      loadScript(REACT_DOM_LOCAL_URL, REACT_DOM_SRI)
    ]).then(() => void 0);
    const loadCdn = () => Promise.all([
      loadScript(REACT_URL, REACT_SRI),
      loadScript(REACT_DOM_URL, REACT_DOM_SRI)
    ]).then(() => void 0);
    return loadLocal().catch(loadCdn);
  }
  function init() {
